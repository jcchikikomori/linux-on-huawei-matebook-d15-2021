# Linux on Huawei MateBook D15 (2021)

> Brain dump: MateBook 15 (BOD-WXX9 or BohrD-WDH9C)
...
I am running Fedora 38 (Workstation Edition)

## Linux Support Matrix

| Device | Model/Spec | Working? (Must be 100%) | Notes |
| --- | --- |  :---: | --- |
| Processor | 11th Gen Intel Core i5-1135G7 | ✔ Yes | 8 cores, power states etc seem to work out of the box |
| Graphics | Iris Xe Graphics | ✔ Yes | via standard kernel driver |
| Memory | 8192 MB | ✔ Yes |  |
| Display | 15.6 inch 16:9 | ✔ Yes | resolution is correctly detected backlight control works via native function keys and can be controlled by KDE settings, see [below](#display) for details on scaling |
| Storage | Samsung 980 NvME SSD, 512 GB | ✔ Yes | via standard kernel driver |
| Wifi | Intel Wireless AX200 (Wifi 6) | ✔ Yes | works out of the box, except currently experiencing intermittent connection |
| Bluetooth | Intel Bluetooth 5.0 | ✔ Yes | works out of the box |
| Soundcard  | Intel Tiger Lake LP Smart Sound Technology | ✔ Yes  | see [below](#soundcard) for details |
| Speakers  |  | ❌ No | Very weak volume, barely audible |
| Microphone | | ❌ No | Same as the speakers |
| Webcam | HD Camera | ✔ Yes | works out of the box, indicating light as well |
| Ports | 1 × USB-C, 1 x USB-A 3.1 & 2 x USB-A 2.0 | ✔ Yes | Works out of the box. Same treatment as Windows like unable to support HDMI on USB-C docks, except MateDock |
| Power button |  | ✔ Yes | needs to be pressed for at least a second to generate event |
| Fingerprint Reader | Goodix XXX | ❌ No | located on the power button, see [below](#fingerprint-reader) for details  |
| Battery | Dynapack | ✔ Yes | see [below](#battery) for details |
...
| Touchpad | ELAN | ✔ Yes | touchpad is detected and works in KDE (though not in Debian installer), [see below](#touchpad) for details |
